...
func NewFVMInterpreter(fvm *FVM, cfg Config) *FVMInterpreter {
	...
	return &FVMInterpreter{
		fvm:      fvm,
		cfg:      cfg,
		gasTable: fvm.ChainConfig().GasTable(fvm.BlockNumber),
	}
}

func (in *FVMInterpreter) enforceRestrictions(op OpCode, operation operation, stack *Stack) error {
	if in.readOnly {
...
func (in *FVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error) {
	...
	if in.cfg.Debug {
		defer func() {
			if err != nil {
				if !logged {
					in.cfg.Tracer.CaptureState(in.fvm, pcCopy, op, gasCopy, cost, mem, stack, contract, in.fvm.depth, err)
				} else {
					in.cfg.Tracer.CaptureFault(in.fvm, pcCopy, op, gasCopy, cost, mem, stack, contract, in.fvm.depth, err)
				}
			}
		}()
	}
	...
	// parent context.
	for atomic.LoadInt32(&in.fvm.abort) == 0 {
		if in.cfg.Debug {
		...
		cost, err = operation.gasCost(in.gasTable, in.fvm, contract, stack, mem, memorySize)
		...
		if in.cfg.Debug {
			in.cfg.Tracer.CaptureState(in.fvm, pc, op, gasCopy, cost, mem, stack, contract, in.fvm.depth, err)
			logged = true
		}
