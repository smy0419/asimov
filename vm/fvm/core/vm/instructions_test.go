...
func testTwoOperandOp(t *testing.T, tests []twoOperandTest, opFn func(pc *uint64, interpreter *FVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error)) {
	var (
		env            = NewFVM(Context{}, nil, chaincfg.ActiveNetParams.FvmParam, Config{})
		stack          = newstack()
		pc             = uint64(0)
		fvmInterpreter = NewFVMInterpreter(env, env.vmConfig)
	)

	env.interpreter = fvmInterpreter
	fvmInterpreter.intPool = poolOfIntPools.get()
	for i, test := range tests {
		...
		stack.push(x)
		stack.push(shift)
		opFn(&pc, fvmInterpreter, nil, nil, stack)
		actual := stack.pop()
		...
		if fvmInterpreter.intPool.pool.len() > 0 {

			poolvals := make(map[*big.Int]struct{})
			poolvals[actual] = struct{}{}

			for fvmInterpreter.intPool.pool.len() > 0 {
				key := fvmInterpreter.intPool.get()
	...
	poolOfIntPools.put(fvmInterpreter.intPool)
}
...
func TestByteOp(t *testing.T) {
	...
	for _, test := range tests {
		...
		stack.push(val)
		stack.push(th)
		opByte(&pc, fvmInterpreter, nil, nil, stack)
		actual := stack.pop()
...
func opBenchmark(bench *testing.B, op func(pc *uint64, interpreter *FVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error), args ...string) {
	var (
		env            = NewFVM(Context{}, nil, chaincfg.ActiveNetParams.FvmParam, Config{})
		stack          = newstack()
		fvmInterpreter = NewFVMInterpreter(env, env.vmConfig)
	...
	for i := 0; i < bench.N; i++ {
		...
		op(&pc, fvmInterpreter, nil, nil, stack)
		stack.pop()
	}
	poolOfIntPools.put(fvmInterpreter.intPool)
}
...
func TestOpMstore(t *testing.T) {
	...
	opMstore(&pc, fvmInterpreter, nil, mem, stack)
