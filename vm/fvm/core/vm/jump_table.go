...
type (
	executionFunc       func(pc *uint64, interpreter *FVMInterpreter, contract *Contract, memory *Memory, stack *Stack) ([]byte, error)
	gasFunc             func(params.GasTable, *FVM, *Contract, *Stack, *Memory, uint64) (uint64, error) // last parameter is the requested memory size as a uint64
...
func newFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		FLOWCREATEASSET: {
			execute:       opFlowCreateAsset,
			gasCost:       gasFlowCreateAsset,
			validateStack: makeStackFunc(3, 0),
			// memorySize:    memoryFlowCreateAsset,
			valid: true,
		},
		FLOWMINTASSET: {
			execute:       opFlowMintAsset,
			gasCost:       gasFlowMintAsset,
			validateStack: makeStackFunc(2, 0),
			// memorySize:    memoryFlowMintAsset,
			valid: true,
		},
		FLOWDEPLOYCONTRACT: {
			execute:       opFlowDeployContract,
			gasCost:       gasFlowDeployContract,
			validateStack: makeStackFunc(3, 1),
			// memorySize:    memoryFlowDeployContract,
			valid:  true,
			writes: true,
		},
		ROUND: {
