...
type serializedKnownAddress struct {
	...
	SrcServices common.ServiceFlag
...
func (a *AddrManager) updateAddress(netAddr, srcAddr *protos.NetAddress) {
...
func (a *AddrManager) getNewBucket(netAddr, srcAddr *protos.NetAddress) int {
	// bitcoind:
	...
	hash1 := common.DoubleHashB(data1)
	...
	hash2 := common.DoubleHashB(data2)
	return int(binary.LittleEndian.Uint64(hash2) % newBucketCount)
}

func (a *AddrManager) getTriedBucket(netAddr *protos.NetAddress) int {
...
func (a *AddrManager) AddAddressByIP(addrIP string) error {
	...
	na := protos.NewNetAddressIPPort(ip, uint16(port), 0)
...
func (a *AddrManager) getAddresses() []*protos.NetAddress {
	a.mtx.Lock()
	defer a.mtx.Unlock()
	...
	addrs := make([]*protos.NetAddress, 0, addrIndexLen)
...
func (a *AddrManager) find(addr *protos.NetAddress) *KnownAddress {
...
func getReachabilityFrom(localAddr, remoteAddr *protos.NetAddress) int {
	const (
		Unreachable = 0
		Default     = iota
		Teredo
...
func (a *AddrManager) GetBestLocalAddress(remoteAddr *protos.NetAddress) *protos.NetAddress {
	...
	var bestAddress *protos.NetAddress
	...
	if bestAddress != nil {
		...
		bestAddress = protos.NewNetAddressIPPort(ip, 0, services)
	}
...
func (a *AddrManager) AddrStringToNetAddr(addr string) (net.Addr, error) {
	...
	if strings.HasSuffix(host, ".onion") {
		if !a.nap.SupportOnion() {
			return nil, errors.New("onion has been disabled")
		}
...
// New returns a new flow address manager.
