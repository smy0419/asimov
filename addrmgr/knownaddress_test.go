...
func TestChance(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			//Test normal case
			addrmgr.TstNewKnownAddress(&protos.NetAddress{Timestamp: now.Add(-35 * time.Second)},
				0, time.Now().Add(-30*time.Minute), time.Now(), false, 0),
			1.0,
		}, {
			//Test case in which lastseen < 0
			addrmgr.TstNewKnownAddress(&protos.NetAddress{Timestamp: now.Add(20 * time.Second)},
				0, time.Now().Add(-30*time.Minute), time.Now(), false, 0),
			1.0,
		}, {
...
func TestIsBad(t *testing.T) {
	...
	futureNa := &protos.NetAddress{Timestamp: future}
	minutesOldNa := &protos.NetAddress{Timestamp: minutesOld}
	monthOldNa := &protos.NetAddress{Timestamp: monthOld}
	currentNa := &protos.NetAddress{Timestamp: secondsOld}
