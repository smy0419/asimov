...
import (
	...
	peerpkg "github.com/AsimovNetwork/asimov/peer"
...
const (
	...
	// hashes to store in memory.
	maxRequestedBlocks = protos.MaxInvPerMsg

	// maxRequestedTxns is the maximum number of requested transactions
	// hashes to store in memory.
	maxRequestedTxns = protos.MaxInvPerMsg
...
type peerSyncState struct {
	requestQueue    []*protos.InvVect
	requestedTxns   map[common.Hash]struct{}
	requestedBlocks map[common.Hash]struct{}
	requestedSigns  map[common.Hash]struct{}
	syncCandidate   bool
	orphanBlocks    int32
}
...
type SyncManager struct {
	...
	rejectedTxns     map[common.Hash]struct{}
	requestedTxns    map[common.Hash]struct{}
	rejectedSigns    map[common.Hash]struct{}
	requestedSigns   map[common.Hash]struct{}
	requestedBlocks  map[common.Hash]struct{}
...
func (sm *SyncManager) resetHeaderState(newestHash *common.Hash, newestHeight int32) {
...
func (sm *SyncManager) startSync() {
	...
	if bestPeer != nil {
		...
		// to send.
		sm.requestedBlocks = make(map[common.Hash]struct{})
...
func (sm *SyncManager) handleNewPeerMsg(peer *peerpkg.Peer) {
	...
	sm.peerStates[peer] = &peerSyncState{
		...
		requestedTxns:   make(map[common.Hash]struct{}),
		requestedBlocks: make(map[common.Hash]struct{}),
		requestedSigns:  make(map[common.Hash]struct{}),
	}
...
func (sm *SyncManager) clearRequestedState(state *peerSyncState) {
	...
	for sigHah := range state.requestedSigns {
		delete(sm.requestedSigns, sigHah)
	}
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	if err != nil {
		...
		// so logger it as an actual error.
		...
		peer.PushRejectMsg(protos.CmdTx, code, reason, txHash, false)
		return
	}
...
func (sm *SyncManager) handleSigMsg(tmsg *sigMsg) {
	...
	state, exists := sm.peerStates[peer]
	if !exists {
		log.Warnf("Received sig message from unknown peer %s", peer)
		return
	}

	sig := tmsg.sig
	sigHash := sig.Hash()
	if _, exists = sm.rejectedSigns[*sigHash]; exists {
		log.Debugf("Ignoring unsolicited previously rejected "+
			"signature %v from %s", sigHash, peer)
		return
	}

	delete(state.requestedSigns, *sigHash)
	delete(sm.requestedSigns, *sigHash)

	bestHeight := sm.chain.BestSnapshot().Height
	// Skip signatures which are too old or too new
	if bestHeight > sig.MsgSign.BlockHeight + 10 || bestHeight < sig.MsgSign.BlockHeight - 10 {
		return
	}

	err := blockchain.AddressVerifySignature(sig.MsgSign.BlockHash[:], &sig.MsgSign.Signer, sig.MsgSign.Signature[:])
	if err != nil {
		log.Debugf("Received sig message with invalid signature")
		sm.pushErrorMsg(peer, sm.rejectedSigns, sigHash, blockchain.RuleError{})
		return
	}

	err = sm.sigMemPool.ProcessSig(tmsg.sig)
	if err != nil {
		sm.pushErrorMsg(peer, sm.rejectedSigns, sigHash, err)
		return
	}

	sm.peerNotifier.AnnounceNewSignature(tmsg.sig)
}

func (sm *SyncManager) pushErrorMsg(peer *peerpkg.Peer, rejectMap map[common.Hash]struct{}, hash *common.Hash, err error) {
	rejectMap[*hash] = struct{}{}
	sm.limitMap(rejectMap, maxRejectedSigns)

	if _, ok := err.(mempool.RuleError); ok {
		log.Debugf("Rejected signature %v from %s: %v",
			hash, peer, err)
	} else {
		//log.Warnf("Failed to process signature %v: %v",
		//	hash, err)
	}
...
func (sm *SyncManager) handleBlockMsg(bmsg *blockMsg) {
	...
	behaviorFlags := common.BFNone
	if sm.headersFirstMode {
		firstNodeEl := sm.headerList.Front()
		if firstNodeEl != nil {
			firstNode := firstNodeEl.Value.(*headerNode)
			if blockHash.IsEqual(firstNode.hash) {
				//behaviorFlags |= common.BFFastAdd
	...
	if err != nil {
		...
		// it as such.  Otherwise, something really did go wrong, so logger
		...
		// send it.
		code, reason := mempool.ErrToRejectErr(err)
		peer.PushRejectMsg(protos.CmdBlock, code, reason, blockHash, false)
		return
	}
	...
	if isOrphan {
		...
		// When the block is not an orphan, logger information about it and
		...
		peer.AcceptBlockHeight(heightUpdate)

		// Clear the rejected transactions.
		sm.rejectedTxns = make(map[common.Hash]struct{})

		if state.orphanBlocks > 0 {
			log.Debugf("Peer %s resetting orphan blocks %d -> 0", peer, state.orphanBlocks)
			state.orphanBlocks = 0
		}
	}
	...
	if sm.nextCheckpoint != nil {
		locator := blockchain.BlockLocator([]*common.Hash{prevHash})
	...
	locator := blockchain.BlockLocator([]*common.Hash{blockHash})
...
func (sm *SyncManager) fetchHeaderBlocks() {
	...
	// describe.  The size hint will be limited to protos.MaxInvPerMsg by
	// the function, so no need to double check it here.
	gdmsg := protos.NewMsgGetDataSizeHint(uint(sm.headerList.Len()))
	numRequested := 0
	for e := sm.startHeader; e != nil; e = e.Next() {
		...
		iv := protos.NewInvVect(protos.InvTypeBlock, node.hash)
		...
		if numRequested >= protos.MaxInvPerMsg {
			break
		}
	}
...
func (sm *SyncManager) handleHeadersMsg(hmsg *headersMsg) {
	...
	// next checkpoint.
	locator := blockchain.BlockLocator([]*common.Hash{finalHash})
	err := peer.PushGetHeadersMsg(locator, sm.nextCheckpoint.Hash)
	if err != nil {
		log.Warnf("Failed to send getheaders message to "+
			"peer %s: %v", peer.Addr(), err)
...
func (sm *SyncManager) haveInventory(invVect *protos.InvVect) (bool, error) {
	switch invVect.Type {
	...
	case protos.InvTypeTx:
		...
		prevOut := protos.OutPoint{Hash: invVect.Hash}
	...
	case protos.InvTypeSignature:
		if sm.sigMemPool.HaveSignature(&invVect.Hash) {
			return true, nil
		}
...
func (sm *SyncManager) handleInvMsg(imsg *invMsg) {
	...
	for i := len(invVects) - 1; i >= 0; i-- {
		if invVects[i].Type == protos.InvTypeBlock {
			lastBlock = i
			break
		}
	}
	...
	for len(requestQueue) != 0 {
		...
		case protos.InvTypeSignature:
			// Request the signature if there is not already a
			// pending request.
			if _, exists := sm.requestedSigns[iv.Hash]; !exists {
				sm.requestedSigns[iv.Hash] = struct{}{}
				sm.limitMap(sm.requestedSigns, maxRequestedSigns)
				state.requestedSigns[iv.Hash] = struct{}{}
...
func (sm *SyncManager) limitMap(m map[common.Hash]struct{}, limit int) {
	if len(m)+1 > limit {
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		...
		for _, tx := range block.Transactions()[:coinbaseIdx] {
		...
		sm.sigMemPool.ConnectSigns(block.Signs(), block.Height())
	...
	case blockchain.NTBlockDisconnected:
		...
		block, ok := blocks[0].(*asiutil.Block)
		if !ok {
			log.Warnf("Chain disconnected notification is not a block.")
			break
		}

		vblock, ok := blocks[1].(*asiutil.VBlock)
		if !ok {
			log.Warnf("Chain disconnected notification is not a vblock.")
			break
		}
		...
		for _, tx := range vblock.Transactions() {
			// Remove all transactions that depend on it if it wasn't accepted into
			// the transaction pool.
			sm.txMemPool.RemoveTransaction(tx, true)
		}
...
func (sm *SyncManager) QueueSig(sig *asiutil.BlockSign, peer *peerpkg.Peer, done chan struct{}) {
	...
	sm.msgChan <- &sigMsg{sig: sig, peer: peer, reply: done}
}
...
func New(config *Config) (*SyncManager, error) {
	sm := SyncManager{
		...
		sigMemPool:       config.SigMemPool,
		chainParams:      config.ChainParams,
		rejectedTxns:     make(map[common.Hash]struct{}),
		requestedTxns:    make(map[common.Hash]struct{}),
		rejectedSigns:    make(map[common.Hash]struct{}),
		requestedSigns:   make(map[common.Hash]struct{}),
		requestedBlocks:  make(map[common.Hash]struct{}),
