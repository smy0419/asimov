...
import (
	...
	"github.com/AsimovNetwork/asimov/winsvc/eventlog"
	"github.com/AsimovNetwork/asimov/winsvc/mgr"
	"github.com/AsimovNetwork/asimov/winsvc/svc"
)

const (
	// svcName is the name of asimovd service.
	svcName = "asimovdsvc"
	...
	svcDisplayName = "Asimovd Service"
...
// logServiceStartOfDay logs information about btcd when the main NodeServer has
// been started to the Windows event log.
func logServiceStartOfDay(srvr *NodeServer) {
	var message string
...
func (s *btcdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// doneChan.  serverChan is notified with the main NodeServer instance once
	// it is started so it can be gracefully stopped.
	doneChan := make(chan error)
	serverChan := make(chan *NodeServer)
	go func() {
...
func installService() error {
	...
	// Support events to the event logger using the standard "standard" Windows
...
// since it would invalidate any existing event logger messages.
...
// the service control manager to start the long-running NodeServer.  A flag is
