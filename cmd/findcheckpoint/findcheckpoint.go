...
func loadBlockDB() (database.Transactor, error) {
	// The database name is based on the database type.
	dbName := blockDbNamePrefix + "_" + cfg.DbType
	dbPath := filepath.Join(cfg.DataDir, dbName)
	fmt.Printf("Loading block database from '%s'\n", dbPath)
	db, err := ffldb.Open(cfg.DbType, dbPath, activeNetParams.Net)
	if err != nil {
		return nil, err
	}
	return db, nil
...
// checkpoints.
func findCandidates(chain *blockchain.BlockChain, latestHash *common.Hash) ([]*chaincfg.Checkpoint, error) {
...
func showCandidate(candidateNum int, checkpoint *chaincfg.Checkpoint) {
	...
	fmt.Printf("Candidate %d -- Round: %d, Hash: %v\n", candidateNum,
