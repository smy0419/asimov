...
func (msg *MsgFilterAdd) VVSDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	var err error
	msg.Data, err = serialization.ReadVarBytes(r, pver, MaxFilterAddDataSize,
		"filterAdd data")
	return err
}
...
func (msg *MsgFilterAdd) VVSEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	if size > MaxFilterAddDataSize {
		...
		return messageError("MsgFilterAdd.VVSEncode", str)
	}
...
func (msg *MsgFilterAdd) MaxPayloadLength(pver uint32) uint32 {
	return uint32(serialization.VarIntSerializeSize(MaxFilterAddDataSize)) +
