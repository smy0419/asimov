...
// command 12 bytes + payload length 4 bytes + checksum 4 bytes.
...
// LatestEncoding is the most recently specified encoding for the Bitcoin protos
// protocol.
...
func readMessageHeader(r io.Reader) (int, *messageHeader, error) {
	// Since serialization.ReadElements doesn't return the amount of bytes read, attempt
	...
	err = serialization.ReadNBytes(hr, command[:], common.CommandSize)
	if err!= nil {
		return n, nil, err
	}

	err = serialization.ReadUint32(hr, &hdr.length)
	if err!= nil {
		return n, nil, err
	}

	err = serialization.ReadNBytes(hr, hdr.checksum[:], 4)
	if err!= nil {
		return n, nil, err
	}
...
func WriteMessageN(w io.Writer, msg Message, pver uint32) (int, error) {
	return WriteMessageWithEncodingN(w, msg, pver, BaseEncoding)
}
...
func WriteMessage(w io.Writer, msg Message, pver uint32) error {
	_, err := WriteMessageN(w, msg, pver)
	return err
}
...
// to specify the message encoding format to be used when serializing protos
// messages.
func WriteMessageWithEncodingN(w io.Writer, msg Message, pver uint32,
	...
	var bw bytes.Buffer
	err := msg.VVSEncode(&bw, pver, encoding)
	if err != nil {
	...
	copy(hdr.checksum[:], common.DoubleHashB(payload)[0:4])

	// Encode the header for the message.  This is done to a buffer
	// rather than directly to the writer since serialization.WriteElements doesn't
	// return the number of bytes written.
	hw := bytes.NewBuffer(make([]byte, 0, MessageHeaderSize))
	err = serialization.WriteNBytes(hw, command[:])
	if err != nil {
		return totalBytes, err
	}

	err = serialization.WriteUint32(hw, hdr.length)
	if err != nil {
		return totalBytes, err
	}

	err = serialization.WriteNBytes(hw, hdr.checksum[:])
	if err != nil {
		return totalBytes, err
	}
...
func ReadMessageWithEncodingN(r io.Reader, pver uint32, enc MessageEncoding) (int, Message, []byte, error) {
	...
	// Test checksum.
	checksum := common.DoubleHashB(payload)[0:4]
	...
	// MsgVersion VVSDecode function requires it.
	pr := bytes.NewBuffer(payload)
	err = msg.VVSDecode(pr, pver, enc)
	if err != nil {
		return totalBytes, nil, nil, err
	}
...
func ReadMessageN(r io.Reader, pver uint32) (int, Message, []byte, error) {
	return ReadMessageWithEncodingN(r, pver, BaseEncoding)
}
...
func ReadMessage(r io.Reader, pver uint32) (Message, []byte, error) {
	_, msg, buf, err := ReadMessageN(r, pver)
	return msg, buf, err
}
