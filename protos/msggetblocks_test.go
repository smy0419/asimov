...
func TestGetBlocks(t *testing.T) {
	...
	locatorHash, err :=  common.NewHashFromStr(hashStr)
	if err != nil {
	...
	hashStop, err :=  common.NewHashFromStr(hashStr)
	if err != nil {
		t.Errorf("NewHashFromStr: %v", err)
	}
...
// TestGetBlocksWire tests the MsgGetBlocks protos encode and decode for various
...
func TestGetBlocksWire(t *testing.T) {
	...
	hashLocator, err :=  common.NewHashFromStr(hashStr)
	if err != nil {
		t.Errorf("NewHashFromStr: %v", err)
	}
	...
	hashLocator2, err :=  common.NewHashFromStr(hashStr)
	if err != nil {
		t.Errorf("NewHashFromStr: %v", err)
	}
	...
	noLocators := NewMsgGetBlocks(&common.Hash{})
	noLocators.ProtocolVersion = pver
	noLocatorsEncoded := []byte{
		0x01, 0x00, 0x00, 0x00, //ProtocolVersion
...
// TestGetBlocksWireErrors performs negative tests against protos encode and
