...
var testGetCFiltersHash = common.Hash{
...
// TestMsgGetCFilters tests the GetCFilters protos encode and decode for various
// numbers of headers and protocol versions.
func TestGetCFiltersSerialize(t *testing.T) {

	getCFilters := NewMsgGetCFilters(1,10,&testGetCFiltersHash)
	getCFiltersEncoded := []byte{
		0x01,                   // filterType.
		0x0a, 0x00, 0x00, 0x00,
		0x6f, 0xe2, 0x8c, 0x0a, 0xb6, 0xf1, 0xb3, 0x72,
		0xc1, 0xa6, 0xa2, 0x46, 0xae, 0x63, 0xf7, 0x4f,
		0x93, 0x1e, 0x83, 0x65, 0xe1, 0x5a, 0x08, 0x9c,
		0x68, 0xd6, 0x19, 0x00, 0x00, 0x00, 0x00, 0x00, // stophash
	}

	tests := []struct {
		in   *MsgGetCFilters     // Message to encode
		out  *MsgGetCFilters     // Expected decoded message
...
func TestMsgGetCFilterErrors(t *testing.T) {
	...
	tests := []struct {
		in       *MsgGetCFilters     // Value to encode
		buf      []byte          // Wire encoding
		pver     uint32          // Protocol version for protos encoding
		enc      MessageEncoding // Message encoding format
		...
		{getCFilters, getCFiltersEncoded, pver, BaseEncoding, 0, io.ErrShortWrite, io.EOF},
		// Force error in startHeight.
		{getCFilters, getCFiltersEncoded, pver, BaseEncoding, 1, io.ErrShortWrite, io.EOF},
		// Force error in stophash.
		{getCFilters, getCFiltersEncoded, pver, BaseEncoding, 5, io.ErrShortWrite, io.EOF},
	}
