...
// TestRejectWire tests the MsgReject protos encode and decode for various
// protocol versions.
func TestRejectWire(t *testing.T) {
	...
	for i, test := range tests {
		...
		var buf bytes.Buffer
		err := test.msg.VVSEncode(&buf, test.pver, test.enc)
		if err != nil {
...
// TestRejectWireErrors performs negative tests against protos encode and decode
// of MsgReject to confirm error paths work correctly.
func TestRejectWireErrors(t *testing.T) {
	//var RejectVersion uint32 = 70002
	pver := common.ProtocolVersion
