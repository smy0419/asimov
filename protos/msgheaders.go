...
func (msg *MsgHeaders) VVSDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockHeadersPerMsg {
		...
		return messageError("MsgHeaders.VVSDecode", str)
	}
...
func (msg *MsgHeaders) VVSEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxBlockHeadersPerMsg {
		str := fmt.Sprintf("too many block headers for message "+
			"[count %v, max %v]", count, MaxBlockHeadersPerMsg)
		return messageError("MsgHeaders.VVSEncode", str)
	}
	...
	for _, bh := range msg.Headers {
		...
		// The protos protocol encoding always includes a 0 for the number
		...
		err = serialization.WriteVarInt(w, pver, 0)
		if err != nil {
			return err
		}
	}
...
func (msg *MsgHeaders) MaxPayloadLength(pver uint32) uint32 {
	...
	return serialization.MaxVarIntPayload + ((BlockHeaderPayload + 1) *
		MaxBlockHeadersPerMsg)
}
