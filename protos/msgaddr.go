...
func (msg *MsgAddr) VVSDecode(r io.Reader, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxAddrPerMsg {
		...
		return messageError("MsgAddr.VVSDecode", str)
	}
...
func (msg *MsgAddr) VVSEncode(w io.Writer, pver uint32, enc MessageEncoding) error {
	...
	if count > MaxAddrPerMsg {
		str := fmt.Sprintf("too many addresses for message "+
			"[count %v, max %v]", count, MaxAddrPerMsg)
		return messageError("MsgAddr.VVSEncode", str)
	}
...
func (msg *MsgAddr) MaxPayloadLength(pver uint32) uint32 {
	...
	return serialization.MaxVarIntPayload + (MaxAddrPerMsg * maxNetAddressPayload)
}
