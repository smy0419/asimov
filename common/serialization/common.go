...
func ReadNBytes(r io.Reader, buf []byte, n int) error {
	...
	if nr != n {
		return errors.New("size unexpected ")
	}
	return nil
}
...
func ReadVarInt(r io.Reader, pver uint32) (uint64, error) {
	...
	case 0xff:
		...
		if rv < min {
			return 0, messageError("serialization.ReadVarInt", "0xff", fmt.Sprintf(
	...
	case 0xfe:
		...
		min := uint64(0x10000)
		if rv < min {
			return 0, messageError("serialization.ReadVarInt", "0xfe", fmt.Sprintf(
				errNonCanonicalVarInt, rv, discriminant, min))
		}
	...
	case 0xfd:
		...
		min := uint64(0xfd)
		if rv < min {
			return 0, messageError("serialization.ReadVarInt", "0xfd", fmt.Sprintf(
				errNonCanonicalVarInt, rv, discriminant, min))
		}
...
func ReadVarString(r io.Reader, pver uint32) (string, error) {
	...
	if count > MaxVarStringPayload {
		str := fmt.Sprintf("variable length string is too long "+
			"[count %d, max %d]", count, MaxVarStringPayload)
		return "", messageError("ReadVarString", "varstring", str)
	}
...
// serialization.ReadVarBytes reads a variable length byte array.  A byte array is encoded
...
func ReadVarBytes(r io.Reader, pver uint32, maxAllowed uint32,
	...
	if count > uint64(maxAllowed) {
		...
		return nil, messageError("serialization.ReadVarBytes", "varBytes", str)
	}
...
func WriteUint16(w io.Writer, e uint16) error {
	return binarySerializer.PutUint16(w, littleEndian, e)
}
...
func WriteUint32B(w io.Writer, e uint32) error {
	return binarySerializer.PutUint32(w, bigEndian, e)
}

func WriteUint64B(w io.Writer, e uint64) error {
	return binarySerializer.PutUint64(w, bigEndian, e)
}
