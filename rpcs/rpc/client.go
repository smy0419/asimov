...
type Client struct {
	// atomic, so must stay 64-bit aligned
	idCounter   uint64
...
func (c *Client) reconnect(ctx context.Context) error {
	...
	if err != nil {
		rpcLog.Tracef("reconnect failed: %v", err)
		return err
	}
	select {
...
func (c *Client) dispatch(conn net.Conn) {
	...
	for {
		...
		case batch := <-c.readResp:
			for _, msg := range batch {
				...
				case msg.isNotification():
					...
					// }})
					rpcLog.Tracef("<-readResp: notification %s", msg)
					c.handleNotification(msg)
				case msg.isResponse():
					// log.Trace("", "msg", log.Lazy{Fn: func() string {
					// 	return fmt.Sprint("<-readResp: response ", msg)
					// }})
					rpcLog.Tracef("<-readResp: response %s", msg)
					c.handleResponse(msg)
				default:
					// log.Debug("", "msg", log.Lazy{Fn: func() string {
					// 	return fmt.Sprint("<-readResp: dropping weird message", msg)
					// }})
					rpcLog.Tracef("<-readResp: dropping weird message %s", msg)
					// TODO: maybe close
				}
			}
		...
		case err := <-c.readErr:
			...
			rpcLog.Debugf("<-readErr, err %v", err)
		...
		case newconn := <-c.reconnected:
			...
			rpcLog.Debugf("<-reconnected, reading %s remote %s", reading, conn.RemoteAddr())
			if reading {
...
func (c *Client) handleNotification(msg *jsonrpcMessage) {
	if !strings.HasSuffix(msg.Method, notificationMethodSuffix) {
		...
		rpcLog.Debugf("dropping non-subscription message , msg %s", msg)
		return
	}
	...
	if err := json.Unmarshal(msg.Params, &subResult); err != nil {
		...
		rpcLog.Debugf("dropping invalid subscription message , msg %s", msg)
		return
	}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	if op == nil {
		...
		rpcLog.Debugf("unsolicited response, msg %s", msg)
		return
	}
