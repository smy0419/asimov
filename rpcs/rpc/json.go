...
// checkReqVersion returns an error when the given reqVersion isn't valid for RPC method calls.
// valid Version's are string
func checkReqVersion(reqVersion string) error {
	if len(reqVersion) == 0 {
		return fmt.Errorf("missing request Version")
	}
	return nil
}
...
func parseRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	//if err := checkReqVersion(in.Version); err != nil {
	//	return nil, false, &invalidMessageError{err.Error()}
	//}
	...
	if strings.HasSuffix(in.Method, subscribeMethodSuffix) {
		...
		if len(in.Payload) > 0 {
			...
			if err := json.Unmarshal(in.Payload, &subscribeMethod); err != nil {
				rpcLog.Debugf("Unable to parse subscription method: %v\n", err)
...
func parseBatchRequest(incomingMsg json.RawMessage) ([]rpcRequest, bool, Error) {
	...
	for i, r := range in {
		...
		//if err := checkReqVersion(r.Version); err != nil {
		//	return nil, false, &invalidMessageError{err.Error()}
		//}
