...
// DialHTTPWithClientAuthorization creates a new RPC client that connects to an RPC server
// over HTTP using the provided HTTP Client and authorization.
func DialHTTPWithClientAuthorization(endpoint string, client *http.Client,
	user, password string) (*Client, error) {
	req, err := http.NewRequest(http.MethodPost, endpoint, nil)
	if err != nil {
		return nil, err
	}
...
func NewHTTPServer(cors []string, vhosts []string, timeouts HTTPTimeouts, srv *Server) *http.Server {
	...
	if timeouts.ReadTimeout < time.Second {
		rpcLog.Warnf("Sanitizing invalid HTTP read timeout, provided %s updated %d", timeouts.ReadTimeout, DefaultHTTPTimeouts.ReadTimeout)
	...
	if timeouts.WriteTimeout < time.Second {
		rpcLog.Warnf("Sanitizing invalid HTTP write timeout, provided %s updated %d", timeouts.WriteTimeout, DefaultHTTPTimeouts.WriteTimeout)
	...
	if timeouts.IdleTimeout < time.Second {
		rpcLog.Warnf("Sanitizing invalid HTTP idle timeout, provided %s updated %d", timeouts.IdleTimeout, DefaultHTTPTimeouts.IdleTimeout)
