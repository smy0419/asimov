...
// DeriveKey is a utility function that derives a key from a common.Hash by
// truncating the bytes of the hash to the appopriate key size.
func DeriveKey(keyHash *common.Hash) [gcs.KeySize]byte {
	var key [gcs.KeySize]byte
...
// key derived from the passed common.Hash using DeriveKey().
func (b *GCSBuilder) SetKeyFromHash(keyHash *common.Hash) *GCSBuilder {
	// Do nothing if the builder's already errored out.
	if b.err != nil {
		return b
	}
...
// AddHash adds a common.Hash to the list of entries to be included in the
// GCS filter when it's built.
func (b *GCSBuilder) AddHash(hash *common.Hash) *GCSBuilder {
	// Do nothing if the builder's already errored out.
	if b.err != nil {
		return b
	}
...
// common.Hash and the passed probability and estimated filter size.
func WithKeyHashPNM(keyHash *common.Hash, p uint8, n uint32,
	m uint64) *GCSBuilder {
...
func BuildBasicFilter(block *protos.MsgBlock, prevOutScripts [][]byte) (*gcs.Filter, error) {
...
func GetFilterHash(filter *gcs.Filter) (common.Hash, error) {
	filterData, err := filter.NBytes()
	if err != nil {
		return common.Hash{}, err
	}

	return common.DoubleHashH(filterData), nil
}

// MakeHeaderForFilter makes a filter chain header for a filter, given the
// filter and the previous filter chain header.
func MakeHeaderForFilter(filter *gcs.Filter, prevHeader common.Hash) (common.Hash, error) {
	filterTip := make([]byte, 2*common.HashLength)
	...
	copy(filterTip[common.HashLength:], prevHeader[:])

	// The final filter hash is the double-sha256 of the hash computed
	// above.
	return common.DoubleHashH(filterTip), nil
}
