...
// protos.MsgMerkleBlock according to a filter.
type merkleBlock struct {
	numTx       uint32
	allHashes   []*common.Hash
	finalHashes []*common.Hash
	matchedBits []byte
	bits        []byte
}
...
// node position.
func (m *merkleBlock) calcHash(height, pos uint32) *common.Hash {
	if height == 0 {
...
// NewMerkleBlock returns a new *protos.MsgMerkleBlock and an array of the matched
// transaction index numbers based on the passed block and filter.
func NewMerkleBlock(block *asiutil.Block, filter *Filter) (*protos.MsgMerkleBlock, []uint32) {
	numTx := uint32(len(block.Transactions()))
	mBlock := merkleBlock{
		numTx:       numTx,
		allHashes:   make([]*common.Hash, 0, numTx),
	...
	msgMerkleBlock := protos.MsgMerkleBlock{
		Header:       block.MsgBlock().Header,
		Transactions: mBlock.numTx,
		Hashes:       make([]*common.Hash, 0, len(mBlock.finalHashes)),
