...
// ln2Squared is simply the square of the natural logger of 2.
...
type Filter struct {
	mtx           sync.Mutex
	msgFilterLoad *protos.MsgFilterLoad
}
...
func NewFilter(elements, tweak uint32, fprate float64, flags protos.BloomUpdateType) *Filter {
	...
	dataLen = minUint32(dataLen, protos.MaxFilterLoadFilterSize*8) / 8
	...
	hashFuncs = minUint32(hashFuncs, protos.MaxFilterLoadHashFuncs)

	data := make([]byte, dataLen)
	msg := protos.NewMsgFilterLoad(data, hashFuncs, tweak, flags)
...
// protos.MsgFilterLoad.
func LoadFilter(filter *protos.MsgFilterLoad) *Filter {
	return &Filter{
...
//
// This function is safe for concurrent access.
func (bf *Filter) Reload(filter *protos.MsgFilterLoad) {
	bf.mtx.Lock()
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) matchesOutPoint(outpoint *protos.OutPoint) bool {
	// Serialize
	var buf [common.HashLength + 4]byte
	copy(buf[:], outpoint.Hash[:])
	binary.LittleEndian.PutUint32(buf[common.HashLength:], outpoint.Index)
...
// AddHash adds the passed common.Hash to the Filter.
//
// This function is safe for concurrent access.
func (bf *Filter) AddHash(hash *common.Hash) {
	bf.mtx.Lock()
	bf.add(hash[:])
	bf.mtx.Unlock()
}
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) addOutPoint(outpoint *protos.OutPoint) {
	// Serialize
	var buf [common.HashLength + 4]byte
	copy(buf[:], outpoint.Hash[:])
	binary.LittleEndian.PutUint32(buf[common.HashLength:], outpoint.Index)
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) maybeAddOutpoint(pkScript []byte, outHash *common.Hash, outIdx uint32) {
	switch bf.msgFilterLoad.Flags {
	case protos.BloomUpdateAll:
		outpoint := protos.NewOutPoint(outHash, outIdx)
		bf.addOutPoint(outpoint)
	case protos.BloomUpdateP2PubkeyOnly:
...
//
// This function MUST be called with the filter lock held.
func (bf *Filter) matchTxAndUpdate(tx *asiutil.Tx) bool {
...
// MsgFilterLoad returns the underlying protos.MsgFilterLoad for the bloom
// filter.
//
// This function is safe for concurrent access.
func (bf *Filter) MsgFilterLoad() *protos.MsgFilterLoad {
	bf.mtx.Lock()
