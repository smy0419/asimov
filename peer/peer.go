...
const (
	...
	MaxProtocolVersion = common.MaxRequestVersion

	// MinAcceptableProtocolVersion is the lowest protocol version that a
	// connected peer may support.
	MinAcceptableProtocolVersion = common.MinRequestVersion
...
type MessageListeners struct {
	...
	OnGetAddr func(p *Peer, msg *protos.MsgGetAddr)

	// OnAddr is invoked when a peer receives an addr bitcoin message.
	OnAddr func(p *Peer, msg *protos.MsgAddr)

	// OnPing is invoked when a peer receives a ping bitcoin message.
	OnPing func(p *Peer, msg *protos.MsgPing)

	// OnPong is invoked when a peer receives a pong bitcoin message.
	OnPong func(p *Peer, msg *protos.MsgPong)

	// OnMemPool is invoked when a peer receives a mempool bitcoin message.
	OnMemPool func(p *Peer, msg *protos.MsgMemPool)

	// OnTx is invoked when a peer receives a tx bitcoin message.
	OnTx func(p *Peer, msg *protos.MsgTx)

	// OnSig is invoked when a peer receives a signature bitcoin message.
	OnSig func(p *Peer, msg *protos.MsgBlockSign)

	// OnBlock is invoked when a peer receives a block bitcoin message.
	OnBlock func(p *Peer, msg *protos.MsgBlock, buf []byte)

	// OnCFilter is invoked when a peer receives a cfilter bitcoin message.
	OnCFilter func(p *Peer, msg *protos.MsgCFilter)

	// OnCFHeaders is invoked when a peer receives a cfheaders bitcoin
	// message.
	OnCFHeaders func(p *Peer, msg *protos.MsgCFHeaders)

	// OnCFCheckpt is invoked when a peer receives a cfcheckpt bitcoin
	// message.
	OnCFCheckpt func(p *Peer, msg *protos.MsgCFCheckpt)

	// OnInv is invoked when a peer receives an inv bitcoin message.
	OnInv func(p *Peer, msg *protos.MsgInv)

	// OnHeaders is invoked when a peer receives a headers bitcoin message.
	OnHeaders func(p *Peer, msg *protos.MsgHeaders)

	// OnNotFound is invoked when a peer receives a notfound bitcoin
	// message.
	OnNotFound func(p *Peer, msg *protos.MsgNotFound)

	// OnGetData is invoked when a peer receives a getdata bitcoin message.
	OnGetData func(p *Peer, msg *protos.MsgGetData)

	// OnGetBlocks is invoked when a peer receives a getblocks bitcoin
	// message.
	OnGetBlocks func(p *Peer, msg *protos.MsgGetBlocks)

	// OnGetHeaders is invoked when a peer receives a getheaders bitcoin
	// message.
	OnGetHeaders func(p *Peer, msg *protos.MsgGetHeaders)

	// OnGetCFilters is invoked when a peer receives a getcfilters bitcoin
	// message.
	OnGetCFilters func(p *Peer, msg *protos.MsgGetCFilters)

	// OnGetCFHeaders is invoked when a peer receives a getcfheaders
	// bitcoin message.
	OnGetCFHeaders func(p *Peer, msg *protos.MsgGetCFHeaders)

	// OnGetCFCheckpt is invoked when a peer receives a getcfcheckpt
	// bitcoin message.
	OnGetCFCheckpt func(p *Peer, msg *protos.MsgGetCFCheckpt)

	// OnFeeFilter is invoked when a peer receives a feefilter bitcoin message.
	OnFeeFilter func(p *Peer, msg *protos.MsgFeeFilter)

	// OnFilterAdd is invoked when a peer receives a filteradd bitcoin message.
	OnFilterAdd func(p *Peer, msg *protos.MsgFilterAdd)

	// OnFilterClear is invoked when a peer receives a filterclear bitcoin
	// message.
	OnFilterClear func(p *Peer, msg *protos.MsgFilterClear)

	// OnFilterLoad is invoked when a peer receives a filterload bitcoin
	// message.
	OnFilterLoad func(p *Peer, msg *protos.MsgFilterLoad)

	// OnMerkleBlock  is invoked when a peer receives a merkleblock bitcoin
	// message.
	OnMerkleBlock func(p *Peer, msg *protos.MsgMerkleBlock)

	// OnVersion is invoked when a peer receives a version bitcoin message.
	OnVersion func(p *Peer, msg *protos.MsgVersion) *protos.MsgReject

	// OnVerAck is invoked when a peer receives a verack bitcoin message.
	OnVerAck func(p *Peer, msg *protos.MsgVerAck)

	// OnReject is invoked when a peer receives a reject bitcoin message.
	OnReject func(p *Peer, msg *protos.MsgReject)

	// OnSendHeaders is invoked when a peer receives a sendheaders bitcoin
	// message.
	OnSendHeaders func(p *Peer, msg *protos.MsgSendHeaders)
	...
	OnRead func(p *Peer, bytesRead int, msg protos.Message, err error)
	...
	OnWrite func(p *Peer, bytesWritten int, msg protos.Message, err error)

	// OnBan is invoked when ban a misbehaving peer.
	OnBan func(p *Peer)
}
...
// information.
func newNetAddress(addr net.Addr, services common.ServiceFlag) (*protos.NetAddress, error) {
	// addr will be a net.TCPAddr when not using a proxy.
	if tcpAddr, ok := addr.(*net.TCPAddr); ok {
		ip := tcpAddr.IP
		port := uint16(tcpAddr.Port)
		na := protos.NewNetAddressIPPort(ip, port, services)
		return na, nil
	}
	...
	na := protos.NewNetAddressIPPort(ip, uint16(port), services)
	return na, nil
}
...
type outMsg struct {
	...
	encoding protos.MessageEncoding
}
...
// HashFunc is a function which returns a block hash, height, weight and error
// It is used as a callback to get newest block details.
type HashFunc func() (hash *common.Hash, height int32, weight uint64, err error)
...
type HostToNetAddrFunc func(host string, port uint16,
	services common.ServiceFlag) (*protos.NetAddress, error)
...
type Peer struct {
	...
	wireEncoding protos.MessageEncoding
	...
	startingHeight     int32
	lastBlock          int32
	lastAnnouncedBlock *common.Hash
	lastPingNonce      uint64    // Set to nonce if we have a pending ping.
	lastPingTime       time.Time // Time we sent last ping.
	lastPingMicros     int64     // Time for last ping to return.
	lastAcceptBlock    int32
	...
	outputInvChan chan *protos.InvVect
	inQuit        chan struct{}
...
func (p *Peer) AcceptBlockHeight(newHeight int32) {
	p.statsMtx.Lock()
	log.Tracef("Updating last accept block height of peer %v from %v to %v",
		p.addr, p.lastAcceptBlock, newHeight)

	if newHeight > p.lastAcceptBlock {
		p.lastAcceptBlock = newHeight
	}
...
func (p *Peer) LastAcceptBlock() int32 {
	p.statsMtx.RLock()
	lastAcceptBlock := p.lastAcceptBlock
	p.statsMtx.RUnlock()

	return lastAcceptBlock
}
...
// remote peer.
func (p *Peer) localVersionMsg() (*protos.MsgVersion, error) {
	var blockNum int32
	if p.cfg.NewestBlock != nil {
		var err error
		_, blockNum, _, err = p.cfg.NewestBlock()
		if err != nil {
			return nil, err
		}
	}
	...
	if p.cfg.Proxy != "" {
		...
		if err != nil || p.na.IP.String() == proxyaddress {
			theirNA = protos.NewNetAddressIPPort(net.IP([]byte{0, 0, 0, 0}), 0, theirNA.Services)
		}
	}

	// Create a protos.NetAddress with only the services set to use as the
	...
	// Version message.
	msg := protos.NewMsgVersion(ourNA, theirNA, nonce, blockNum, p.cfg.ChainParams.Net)
...
func (p *Peer) PushAddrMsg(addresses []*protos.NetAddress) ([]*protos.NetAddress, error) {
	...
	msg := protos.NewMsgAddr()
	msg.AddrList = make([]*protos.NetAddress, addressCount)
	copy(msg.AddrList, addresses)

	// Randomize the addresses sent if there are more than the maximum allowed.
	if addressCount > protos.MaxAddrPerMsg {
		// Shuffle the address list.
		for i := 0; i < protos.MaxAddrPerMsg; i++ {
			j := i + rand.Intn(addressCount-i)
			msg.AddrList[i], msg.AddrList[j] = msg.AddrList[j], msg.AddrList[i]
		}

		// Truncate it to the maximum size.
		msg.AddrList = msg.AddrList[:protos.MaxAddrPerMsg]
	}
...
func (p *Peer) PushGetBlocksMsg(locator blockchain.BlockLocator, stopHash *common.Hash) error {
	...
	msg := protos.NewMsgGetBlocks(stopHash)
...
func (p *Peer) PushRejectMsg(command string, code protos.RejectCode, reason string, hash *common.Hash, wait bool) {
	...
	msg := protos.NewMsgReject(command, code, reason)
	if command == protos.CmdTx || command == protos.CmdBlock {
		if hash == nil {
...
func (p *Peer) handleRemoteVersionMsg(msg *protos.MsgVersion) error {
	...
	// protos.RejectVersion, this should send a reject packet before
	// disconnecting.
...
func (p *Peer) handlePingMsg(msg *protos.MsgPing) {
	// Include nonce from ping so pong can be identified.
	p.QueueMessage(protos.NewMsgPong(msg.Nonce), nil)
}
...
func (p *Peer) handlePongMsg(msg *protos.MsgPong) {
	p.statsMtx.Lock()
...
func (p *Peer) readMessage(encoding protos.MessageEncoding) (protos.Message, []byte, error) {
	n, msg, buf, err := protos.ReadMessageWithEncodingN(p.conn, p.ProtocolVersion(), encoding)
	...
	// Use closures to logger expensive operations so they are only run when
...
func (p *Peer) writeMessage(msg protos.Message, enc protos.MessageEncoding) error {
	...
	log.Tracef("%v", newLogClosure(func() string {
		...
		_, err := protos.WriteMessageWithEncodingN(&buf, msg, p.ProtocolVersion(), enc)
		if err != nil {
			return err.Error()
		}
...
// response for the passed protos protocol command to the pending responses map.
func (p *Peer) maybeAddDeadline(pendingResponses map[string]time.Time, msgCmd string) {
	...
	switch msgCmd {
	case protos.CmdVersion:
		// Expects a verack message.
		pendingResponses[protos.CmdVerAck] = deadline

	case protos.CmdMemPool:
		// Expects an inv message.
		pendingResponses[protos.CmdInv] = deadline

	case protos.CmdGetBlocks:
		// Expects an inv message.
		pendingResponses[protos.CmdInv] = deadline

	case protos.CmdGetData:
		// Expects a block, merkleblock, tx, or notfound message.
		pendingResponses[protos.CmdBlock] = deadline
		pendingResponses[protos.CmdMerkleBlock] = deadline
		pendingResponses[protos.CmdTx] = deadline
		pendingResponses[protos.CmdNotFound] = deadline

	case protos.CmdGetHeaders:
		// Expects a headers message.  Use a longer deadline since it
		// can take a while for the remote peer to load all of the
		// headers.
		deadline = time.Now().Add(stallResponseTimeout * 3)
		pendingResponses[protos.CmdHeaders] = deadline
	}
}
	...
	for {
		...
		case msg := <-p.stallControl:
			...
			case sccReceiveMessage:
				...
				case protos.CmdBlock:
					fallthrough
				case protos.CmdMerkleBlock:
					fallthrough
				case protos.CmdTx:
					fallthrough
				case protos.CmdNotFound:
					delete(pendingResponses, protos.CmdBlock)
					delete(pendingResponses, protos.CmdMerkleBlock)
					delete(pendingResponses, protos.CmdTx)
					delete(pendingResponses, protos.CmdNotFound)
	...
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		if err != nil {
			...
			if p.shouldHandleReadError(err) {
				...
				// currently exposed by protos, so just used malformed for the
				// command.
				p.PushRejectMsg("malformed", protos.RejectMalformed, errMsg, nil,
					true)
			}
			break out
		}
		...
		case *protos.MsgVersion:
			...
			p.PushRejectMsg(msg.Command(), protos.RejectDuplicate,
		...
		case *protos.MsgBlockSign:
			if p.cfg.Listeners.OnSig != nil {
				p.cfg.Listeners.OnSig(p, msg)
			}
...
out:
	for {
		...
		case <-pingTicker.C:
			...
			p.QueueMessage(protos.NewMsgPing(nonce), nil)
...
func (p *Peer) QueueMessage(msg protos.Message, doneChan chan<- struct{}) {
	p.QueueMessageWithEncoding(msg, doneChan, protos.BaseEncoding)
}

// QueueMessageWithEncoding adds the passed bitcoin message to the peer send
// queue. This function is identical to QueueMessage, however it allows the
// caller to specify the protos encoding type that should be used when
// encoding/decoding blocks and transactions.
//
// This function is safe for concurrent access.
func (p *Peer) QueueMessageWithEncoding(msg protos.Message, doneChan chan<- struct{},
	encoding protos.MessageEncoding) {
...
func (p *Peer) AssociateConnection(conn net.Conn) {
	...
	p.isWhitelisted = isWhitelisted(conn.RemoteAddr())
	if p.inbound {
...
func (p *Peer) start() error {
	...
	p.QueueMessage(protos.NewMsgVerAck(), nil)
	return nil
}
...
func (p *Peer) readRemoteVersionMsg() error {
	...
	msg, _, err := p.readMessage(protos.LatestEncoding)
	if err != nil {
		return err
	}

	remoteVerMsg, ok := msg.(*protos.MsgVersion)
	if !ok {
		errStr := "A version message must precede all others"
		log.Errorf(errStr)

		rejectMsg := protos.NewMsgReject(msg.Command(), protos.RejectMalformed,
			errStr)
		return p.writeMessage(rejectMsg, protos.LatestEncoding)
	}
...
func (p *Peer) writeLocalVersionMsg() error {
	...
	return p.writeMessage(localVerMsg, protos.LatestEncoding)
}
...
func (p *Peer) AddBanScore(persistent, transient uint32, reason string) {
	...
	warnThreshold := chaincfg.Cfg.BanThreshold >> 1
	...
	if score > warnThreshold {
		log.Warnf("Misbehaving peer %s: %s -- ban score increased to %d",
			p, reason, score)
		if score > chaincfg.Cfg.BanThreshold {
			log.Warnf("Misbehaving peer %s -- banning and disconnecting",
				p)
			if p.cfg.Listeners.OnBan != nil {
				p.cfg.Listeners.OnBan(p)
			}
			p.Disconnect()
		}
	}
...
func newPeerBase(origCfg *Config, inbound bool) *Peer {
	...
	p := Peer{
		inbound:         inbound,
		wireEncoding:    protos.BaseEncoding,
		knownInventory:  newMruInventoryMap(maxKnownInventory),
		stallControl:    make(chan stallControlMsg, 1), // nonblocking sync
		outputQueue:     make(chan outMsg, outputBufferSize),
		sendQueue:       make(chan outMsg, 1),   // nonblocking sync
		sendDoneQueue:   make(chan struct{}, 1), // nonblocking sync
		outputInvChan:   make(chan *protos.InvVect, outputBufferSize),
...
func NewOutboundPeer(cfg *Config, addr string) (*Peer, error) {
	...
	if cfg.HostToNetAddress != nil {
		...
		p.na = protos.NewNetAddressIPPort(net.ParseIP(host), uint16(port), 0)
	}
...
func isWhitelisted(addr net.Addr) bool {
	if len(chaincfg.Cfg.Whitelists) == 0 {
		return false
	}
	...
	for _, ipnet := range chaincfg.Cfg.Whitelists {
