...
func TestMruInventoryMap(t *testing.T) {
	...
	// inventory code.
	numInvVects := 10
	invVects := make([]*protos.InvVect, 0, numInvVects)
	for i := 0; i < numInvVects; i++ {
		hash := &common.Hash{byte(i)}
		iv := protos.NewInvVect(protos.InvTypeBlock, hash)
...
testLoop:
	for i, test := range tests {
		...
		if test.limit > 1 {
			...
			iv := protos.NewInvVect(protos.InvTypeBlock,
				&common.Hash{0x00, 0x01})
			mruInvMap.Add(iv)
...
func TestMruInventoryMapStringer(t *testing.T) {
	...
	iv1 := protos.NewInvVect(protos.InvTypeBlock, hash1)
	iv2 := protos.NewInvVect(protos.InvTypeBlock, hash2)
