...
const (
	...
	// statusFirstInRound indicates that the block is the first one in that
	// round.
	statusFirstInRound
...
func initBlockNode(node *blockNode, roundNode *ainterface.Round, blockHeader *protos.BlockHeader, parent *blockNode) {
	*node = blockNode{
		...
		poaHash:  blockHeader.PoaHash,
		hash:     blockHeader.BlockHash(),
		weight:   uint64(blockHeader.Weight),
	}
	if parent != nil {
		node.parent = parent
...
func newBlockNode(roundNode *ainterface.Round, blockHeader *protos.BlockHeader, parent *blockNode) *blockNode {
	var node blockNode
	initBlockNode(&node, roundNode, blockHeader, parent)
	return &node
}
...
func (node *blockNode) Header() protos.BlockHeader {
	// No lock is needed because all accessed fields are immutable.
	prevHash := &zeroHash
	weight := node.weight
	if node.parent != nil {
		prevHash = &node.parent.hash
		weight = weight - node.parent.weight
	}
...
func (node *blockNode) CalcPastMedianTime() int64 {
	...
	// will always be an odd number of blocks in the set per the common.
	//
...
func (node *blockNode) GetTime() int64 {
	return node.timestamp
}

func (node *blockNode) Hash() common.Hash {
	return node.hash
}

func (node *blockNode) StateRoot() common.Hash {
	return node.stateRoot
}

func (node *blockNode) Coinbase() common.Address {
	return node.coinbase
}

func (node *blockNode) Round() uint32 {
	return node.round.Round
}

func (node *blockNode) Slot() uint16 {
	return node.slot
}
...
func (node *blockNode) GasUsed() uint64 {
	return node.gasUsed
}

func (node *blockNode) GasLimit() uint64 {
	return node.gasLimit
}
...
type blockIndex struct {
	...
	sync.RWMutex
	index map[common.Hash]*blockNode
...
// manually added.
func newBlockIndex(db database.Transactor, chainParams *chaincfg.Params) *blockIndex {
	return &blockIndex{
		db:          db,
		chainParams: chainParams,
		index:       make(map[common.Hash]*blockNode),
...
//
// This function is safe for concurrent access.
func (bi *blockIndex) HaveBlock(hash *common.Hash) bool {
	bi.RLock()
...
//
// This function is safe for concurrent access.
func (bi *blockIndex) LookupNode(hash *common.Hash) *blockNode {
	bi.RLock()
	node := bi.index[*hash]
	bi.RUnlock()
	return node
