...
func chainedNodes(parent *blockNode, numNodes int, chainIdx uint32) []*blockNode {
	nodes := make([]*blockNode, numNodes)
	tip := parent
	round := &ainterface.Round{
		Round:chainIdx + 1,
	...
	for i := 0; i < numNodes; i++ {
		...
		header.Round = chainIdx + 1 //only for test:
		if tip != nil {
			header.PrevBlock = tip.hash
		}
		header.Timestamp = round.RoundStartUnix + round.Duration * int64(i+1)
		nodes[i] = newBlockNode(round, &header, tip)
		tip = nodes[i]
	}
	return nodes
}
...
func TestChainView(t *testing.T) {
	...
	//             \-> 3a'-> 4a' -> 5a'
	branch0Nodes := chainedNodes(nil, 5, 0)
	branch1Nodes := chainedNodes(branch0Nodes[1], 25,1)
	branch2Nodes := chainedNodes(branch1Nodes[0], 3,2)
...
testLoop:
	for _, test := range tests {
		...
		if test.view.Height() != test.tip.height {
			t.Errorf("%s: unexpected active view height -- got  %d, want %d",
				test.name, test.view.Height(), test.tip.height)
			continue
		}
		if test.side.Height() != test.sideTip.height {
			t.Errorf("%s: unexpected side view height -- got %d, want %d",
				test.name, test.side.Height(), test.sideTip.height)
			continue
		}
		...
		if test.view.Genesis() != test.genesis {
			t.Errorf("%s: unexpected active view genesis -- got %v, want %v",
				test.name, test.view.Genesis(), test.genesis)
			continue
		}
		...
		if test.view.Tip() != test.tip {
			t.Errorf("%s: unexpected active view tip -- got %v, want %v",
				test.name, test.view.Tip(), test.tip)
			continue
		}
		if test.side.Tip() != test.sideTip {
			t.Errorf("%s: unexpected active view tip -- got %v, want %v",
				test.name, test.side.Tip(), test.sideTip)
			continue
		}
		...
		if forkNode != test.fork {
			t.Errorf("%s: unexpected fork node (view, side) -- got %v, want %v",
				test.name, forkNode, test.fork)
			continue
		}
		forkNode = test.side.FindFork(test.view.Tip())
		if forkNode != test.fork {
			t.Errorf("%s: unexpected fork node (side, view) -- got %v, want %v",
				test.name, forkNode, test.fork)
			continue
		}
		...
		if forkNode != test.view.Tip() {
			t.Errorf("%s: unexpected fork node (view, tip) -- got %v, want %v",
				test.name, forkNode, test.view.Tip())
			continue
		}
		...
		for _, node := range test.contains {
			if !test.view.Contains(node) {
				t.Errorf("%s: expected %v in active view", test.name, node)
				continue testLoop
			}
		}
		...
		for _, node := range test.noContains {
			if test.view.Contains(node) {
				t.Errorf("%s: unexpected %v in active view", test.name, node)
				continue testLoop
			}
		}
		...
		for i, node := range test.contains {
			...
			if next := test.view.Next(node); next != expected {
				t.Errorf("%s: unexpected next node -- got %v, want %v", test.name, next, expected)
				continue testLoop
			}
		}
		...
		for _, node := range test.noContains {
			if next := test.view.Next(node); next != nil {
				t.Errorf("%s: unexpected next node -- got %v, want nil", test.name, next)
				continue testLoop
			}
		}
		...
		for _, wantNode := range test.contains {
			...
			if node != wantNode {
				t.Errorf("%s: unexpected node for height %d -- got %v, want %v",
					test.name, wantNode.height, node, wantNode)
				continue testLoop
			}
		}
...
func TestChainViewForkCorners(t *testing.T) {
	...
	branchNodes := chainedNodes(nil, 5,0)
	unrelatedBranchNodes := chainedNodes(nil, 7,1)
	...
	for _, test := range tests {
		for i, tip := range test.tips {
			...
			if test.view.Tip() != tip {
				t.Errorf("%s: unexpected view tip -- got %v, want %v", test.name, test.view.Tip(), tip)
				continue testLoop
			}
...
func TestChainViewNil(t *testing.T) {
	...
	if genesis := view.Genesis(); genesis != nil {
		t.Fatalf("Genesis: unexpected genesis -- got %v, want nil", genesis)
	}
	...
	if height := view.Height(); height != -1 {
		t.Fatalf("Round: unexpected height -- got %d, want -1", height)
	}
	...
	fakeNode := chainedNodes(nil, 1,1)[0]
	...
	if locator := view.BlockLocator(nil); locator != nil {
		t.Fatalf("BlockLocator: unexpected locator -- got %v, want nil", locator)
	}

	// Ensure attempting to get a block locator for a node that exists still
	// works as intended.
	branchNodes := chainedNodes(nil, 50,1)
