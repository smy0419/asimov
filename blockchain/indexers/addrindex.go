...
const (
	...
	addrKeySize = common.AddressLength
	...
	// Size of a transaction entry.  It consists of 4 bytes block id + 1 byte
	// block type + 4 bytes offset + 4 bytes length.
	txEntrySize = 4 + 1 + 4 + 4

	templateEntrySize = 12
)
...
//
// The approach used to store the index is similar to a logger-structured merge
...
type fetchBlockHashFunc func(serializedID []byte) (*common.Hash, error)
...
func serializeAddrIndexEntry(blockID uint32, blockType database.BlockType, txLoc protos.TxLoc) []byte {
	// Serialize the entry.
	serialized := make([]byte, txEntrySize)
	byteOrder.PutUint32(serialized, blockID)
	serialized[4] = byte(blockType)
	byteOrder.PutUint32(serialized[5:], uint32(txLoc.TxStart))
	byteOrder.PutUint32(serialized[9:], uint32(txLoc.TxLen))
	return serialized
}
...
func deserializeAddrIndexEntry(serialized []byte, region *database.BlockRegion, fetchBlockHash fetchBlockHashFunc) error {
	...
	if len(serialized) < txEntrySize {
		return common.DeserializeError("unexpected end of data")
	}
	...
	region.Key = database.NewBlockKey(hash, database.BlockType(serialized[4]))
	region.Offset = byteOrder.Uint32(serialized[5:9])
	region.Len = byteOrder.Uint32(serialized[9:13])
	return nil
}
...
func dbPutAddrIndexEntry(bucket internalBucket, addrKey [addrKeySize]byte, blockID uint32, blockType database.BlockType, txLoc protos.TxLoc) error {
	...
	newData := serializeAddrIndexEntry(blockID, blockType, txLoc)
...
func dbRemoveAddrIndexEntries(bucket internalBucket, addrKey [addrKeySize]byte, count int) error {
	...
	for level := uint8(0); numRemaining > 0; level++ {
		...
		if len(curLevelData) == 0 && numRemaining > 0 {
			return common.AssertError(fmt.Sprintf("dbRemoveAddrIndexEntries "+
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[common.Hash]*asiutil.Tx
	addrsByTx       map[common.Hash]map[[addrKeySize]byte]struct{}
}

// Ensure the AddrIndex type implements the Indexer interface.
var _ blockchain.Indexer = (*AddrIndex)(nil)
...
func (idx *AddrIndex) indexBlock(data writeIndexData, block *asiutil.Block,
	...
	for txIdx, tx := range block.Transactions() {
		...
		for ; vtxIdx < vtxLength; vtxIdx++ {
			vtx := vblock.MsgVBlock().VTransactions[vtxIdx]
			if vtx.Version < uint32(txIdx) {
				continue
			}
			if vtx.Version > uint32(txIdx) {
				break
			...
			// already been proven on the first transaction in the block is
			// a coinbase.
			for _, vtxin := range vtx.TxIn {
				if asiutil.IsMintOrCreateInput(vtxin) {
					continue
				}
				// We'll access the slice of all the
				// transactions spent in this block properly
				// ordered to fetch the previous input script.
				pkScript := stxos[stxoIndex].PkScript
				idx.indexPkScript(vdata, pkScript, vtxIdx)
			...
			for _, txOut := range vtx.TxOut {
				idx.indexPkScript(vdata, txOut.PkScript, vtxIdx)
			}
		}
	}
	// Add auto merge vtx.
...
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *asiutil.Block,
	...
	addrsToVTxns := make(writeIndexData)
	idx.indexBlock(addrsToTxns, block, stxos, addrsToVTxns, vblock)
	...
	for addrKey, txIdxs := range addrsToTxns {
		for _, txIdx := range txIdxs {
			err := dbPutAddrIndexEntry(addrIdxBucket, addrKey,
				blockID, database.BlockNormal, txLocs[txIdx])
			if err != nil {
				return err
			}
		}
	...
	for addrKey, txIdxs := range addrsToVTxns {
		for _, txIdx := range txIdxs {
			err := dbPutAddrIndexEntry(addrIdxBucket, addrKey,
				blockID, database.BlockVirtual, vtxLocs[txIdx])
			if err != nil {
				return err
			}
		}
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr common.IAddress, numToSkip, numRequested uint32, reverse bool) ([]database.BlockRegion, uint32, error) {
	...
	err := idx.db.View(func(dbTx database.Tx) error {
		...
		fetchBlockHash := func(id []byte) (*common.Hash, error) {
			// Deserialize and populate the result.
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *asiutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[common.Hash]*asiutil.Tx)
...
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr common.IAddress) []*asiutil.Tx {
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*asiutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.Transactor) *AddrIndex {
	...
	return &AddrIndex{
		db:         db,
		txnsByAddr: make(map[[addrKeySize]byte]map[common.Hash]*asiutil.Tx),
		addrsByTx:  make(map[common.Hash]map[[addrKeySize]byte]struct{}),
	}
}
