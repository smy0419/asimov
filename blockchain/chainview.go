...
// in a day on average.
const approxNodesPerDay = 12 * 60 * 24
...
// Round returns the height of the tip of the chain view.  It will return -1 if
// there is no tip (which only happens if the chain view has not been
// initialized).
//
// This function is safe for concurrent access.
...
func (c *chainView) findFork(node *blockNode) *blockNode {
	...
	// the Ancestor function such as using an O(logger n) skip list.
