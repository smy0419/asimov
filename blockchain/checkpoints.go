...
func (b *BlockChain) verifyCheckpoint(height int32, hash *common.Hash) bool {
	if !b.HasCheckpoints() {
		return true
	}
...
func (b *BlockChain) findPreviousCheckpoint() (*blockNode, error) {
	...
	if checkpointNode == nil {
		return nil, common.AssertError(fmt.Sprintf("findPreviousCheckpoint "+
...
func isNonstandardTransaction(tx *asiutil.Tx) bool {
...
func (b *BlockChain) IsCheckpointCandidate(block *asiutil.Block) (bool, error) {
	...
	if node.height != block.Height() {
		errStr := fmt.Sprintf("passed block height of %d does not "+
			"match the main chain height of %d", block.Height(),
			node.height)
		return false, ruleError(ErrNotInMainChain,errStr)
	}
	...
	// not always the case).
	prevTime := node.parent.timestamp
	curTime := block.MsgBlock().Header.Timestamp
	nextTime := nextNode.timestamp
	if prevTime > curTime || nextTime < curTime {
		return false, nil
	}
