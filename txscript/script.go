...
func isScriptHash(pops []parsedOpcode) bool {
	return len(pops) == 3 &&
		...
		pops[1].opcode.value == OP_DATA_21 &&
		pops[1].data[0] == common.ScriptHashAddrID &&
		pops[2].opcode.value == OP_IFLAG_EQUAL
}
...
func IsVtx(sigscript []byte) bool {
	return len(sigscript) > 0 && sigscript[0] == OP_SPEND
}
...
// engine instance, calculate the signature hash to be used for signing and
// verification.
func calcSignatureHash(script []parsedOpcode, hashType SigHashType, tx *protos.MsgTx, idx int) []byte {
	...
	//
	// A bug in the original Xing client implementation means specifying
...
func IsUnspendable(pkScript []byte) bool {
	...
	return scriptClass == NonStandardTy || scriptClass == MultiSigTy
}
