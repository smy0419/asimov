	...
	tests := []struct {
		...
		{// #5
			...
			pkScript: []byte{
				...
				0x15,
				// <21-byte script hash>
				0x63, 0x6f, 0x7a, 0x5a, 0xa8, 0xf2, 0xb1, 0x0c,
				0xa5, 0x15, 0x04, 0x52, 0x3a, 0x60, 0xd4, 0x03,
				0x06, 0xf6, 0x96, 0xcd, 0x06,
			},
			valid: true,
		},
		{// #6
			name: "valid create P2Contract",
			pkScript: []byte{
				// OP_CREATE
				0xc1,
			},
	...
	for i, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			case err != nil && test.valid:
				t.Fatalf("#%v, unable to parse valid pkScript=%x: %v", i,
					test.pkScript, err)
			case err == nil && !test.valid:
				t.Fatalf("#%v, successfully parsed invalid pkScript=%x", i,
					test.pkScript)
			}
			...
			if !bytes.Equal(pkScript.Script(), test.pkScript) {
				t.Fatalf("#%v, expected to re-derive pkScript=%x, "+
					"got pkScript=%x", i, test.pkScript,
					pkScript.Script())
			}
		})
	}
...
func TestComputePkScript(t *testing.T) {
	...
	for i, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil && valid {
				t.Fatalf("#%v, unable to compute pkScript: %v", i, err)
			}
			...
			if pkScript.Class() != test.class {
				t.Fatalf("#%v, expected pkScript of type %v, got %v", i,
					test.class, pkScript.Class())
			}
			if !bytes.Equal(pkScript.Script(), test.pkScript) {
				t.Fatalf("#%v, expected pkScript=%x, got pkScript=%x", i,
