...
const (
	...
	// callLen is the length of a P2SH script.
	callLen = 23

	// createLen is the length of a P2SH script.
	createLen = 1
...
func ParsePkScript(pkScript []byte) (PkScript, error) {
	...
	scriptClass, _, _, err := ExtractPkScriptAddrs(pkScript)
	if err != nil {
...
func (s PkScript) Script() []byte {
	...
	case CallTy:
		script = make([]byte, callLen)
		copy(script, s.script[:callLen])

	case CreateTy, TemplateTy:
		script = make([]byte, createLen)
		copy(script, s.script[:createLen])
...
func (s PkScript) Address() (common.IAddress, error) {
	_, addrs, _, err := ExtractPkScriptAddrs(s.Script())
	if err != nil {
...
func ComputePkScript(sigScript []byte) (PkScript, error) {
	...
	case len(sigScript) == pubKeyHashSigScriptLen ||
		...
		if crypto.IsCompressedPubKey(pubKey) {
			...
			pubKeyHashAddr, err := common.NewAddressWithId(common.PubKeyHashAddrID, pubKeyHash)
			if err != nil {
				return pkScript, err
			}
			script, err := payToPubKeyHashScript(pubKeyHashAddr.ScriptAddress())
			if err != nil {
				return pkScript, err
			}
	...
	case IsPushOnlyScript(sigScript):
		...
		pubKeyHashAddr, err := common.NewAddressWithId(common.ScriptHashAddrID, scriptHash)
		if err != nil {
			return pkScript, err
		}
		script, err := payToScriptHashScript(pubKeyHashAddr.ScriptAddress())
		if err != nil {
			return pkScript, err
		}
