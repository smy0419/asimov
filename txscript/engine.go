...
func (vm *Engine) executeOpcode(pop *parsedOpcode) error {
	...
	// Nothing left to do and it is not in an executing branch.
...
func (vm *Engine) Step() (done bool, err error) {
	...
	if vm.scriptOff >= len(vm.scripts[vm.scriptIdx]) {
		...
		// Alt len(virtual-machine.scripts[virtual-machine.scriptIdx])stack doesn't persist.
		_ = vm.astack.DropN(vm.astack.Depth())

		vm.numOps = 0 // number of ops is per script.
		vm.scriptOff = 0
		if vm.scriptIdx == 0 && vm.scriptHashFlag {
			vm.scriptIdx++
			vm.savedFirstStack = vm.GetStack()
		} else if vm.scriptIdx == 1 && vm.scriptHashFlag {
...
func (vm *Engine) checkSignatureEncoding(sig []byte) error {
	...
	if rLen == 0 {
		return scriptError(ErrSigZeroRLen,
			"malformed signature: R length is zero")
	}

	// R must not be negative.
	if sig[rOffset]&0x80 != 0 {
		return scriptError(ErrSigNegativeR,
	...
	if sLen == 0 {
		return scriptError(ErrSigZeroSLen,
			"malformed signature: S length is zero")
	}

	// S must not be negative.
	if sig[sOffset]&0x80 != 0 {
		return scriptError(ErrSigNegativeS,
...
func NewEngine(scriptPubKey []byte, tx *protos.MsgTx, txIdx int, flags ScriptFlags,
	...
	// when it should be.
	vm := Engine{flags: flags, inputAmount: inputAmount, height: height}
