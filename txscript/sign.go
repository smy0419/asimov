...
func SignatureScript(tx *protos.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *crypto.PrivateKey, compress bool) ([]byte, error) {
	...
	pk := (*crypto.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
func p2pkSignatureScript(tx *protos.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *crypto.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *protos.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []common.IAddress, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(tx *protos.MsgTx, idx int,
	subScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB) ([]byte,
	ScriptClass, []common.IAddress, int, error) {
	...
	case MultiSigTy:
		...
		return script, class, addresses, nrequired, nil
	case CallTy, CreateTy, TemplateTy:
		return nil, class, nil, 0,
			errors.New("no need sign Contract transactions")
	default:
		return nil, class, nil, 0,
...
func mergeScripts(tx *protos.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []common.IAddress,
	...
	case ScriptHashTy:
		...
		class, addresses, nrequired, _ := ExtractPkScriptAddrs(script)
		...
		// Merge
		mergedScript := mergeScripts(tx, idx, script,
...
func mergeMultiSig(tx *protos.MsgTx, idx int, addresses []common.IAddress,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := crypto.ParseDERSignature(tSig, crypto.S256())
		if err != nil {
			continue
		}
...
func SignTxOutput(tx *protos.MsgTx, idx int,
	...
	if class == ScriptHashTy {
		realSigScript, _, _, _, err := sign(tx, idx,
	...
	mergedScript := mergeScripts(tx, idx, pkScript, class,
