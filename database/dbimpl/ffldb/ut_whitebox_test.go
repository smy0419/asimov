...
var (
	...
	// of the block chain.
	blockDataFile = filepath.Join("testdata", "blocks1.gz")
...
// a slice of them.
func loadBlocks(t *testing.T, dataFile string, network common.AsimovNet) ([]*asiutil.Block, error) {
	...
	dr, err := zlib.NewReader(fi)

	// Set the first block as the genesis block.
	blocks := make([]*asiutil.Block, 0, 256)
	genesis := asiutil.NewBlock(chaincfg.MainNetParams.GenesisBlock)
	...
	for height := 1; ; height++ {
		...
		if err != nil {
			t.Errorf("Failed to load block %d, len %d: %v", height, blockLen, err)
			return nil, err
		}
...
func TestConvertErr(t *testing.T) {
	...
	for i, test := range tests {
		gotErr := database.ConvertErr("test", test.err)
...
//				err := tx.StoreBlock(block.Hash(), blockBytes)
//				if err != nil {
...
//	err = tc.db.Update(func(tx database.Tx) error {
//		blockBytes, _ := tc.blocks[0].Bytes()
//		bytesLen := len(blockBytes)
//		if bytesLen > maxBytes {
//			bytesLen = maxBytes
//		}
//
//		err := tx.StoreBlock(tc.blocks[0].Hash(), blockBytes[:bytesLen])
//		if err != nil {
...
//		}
//
//		err := tx.StoreBlock(tc.blocks[0].Hash(), blockBytes[:blockSize])
//		if err != nil {
//			tc.t.Errorf("StoreBlock: unexpected error: %v", err)
//			return errSubTestFail
//		}
...
//		// fix temp
//		{uint32(blockSize) + 10, false, database.ErrCorruption},
//	}
//	err = tc.db.View(func(tx database.Tx) error {
//		// fix temp
//		// change tc.files[0] to tc.files[1]
//		// idx rules defined in blockio.go line 420
//		// idx = 0 when blockSize <= 1024
//		// idx = 1 when blockSize > 1024
//		data := tc.files[1].file.(*mockFile).data
//		for i, test := range tests {
...
//	_ = os.RemoveAll(dbPath)
//	idb, err := Create(dbType, dbPath, blockDataNet)
//	if err != nil {
...
//		if fileNum == ^uint32(0) {
//			return nil, database.MakeError(database.ErrDriverSpecific,
//				"test", nil)
//		}
//		if file, ok := tc.files[fileNum]; ok {
//			// "Reopen" the file.
...
//		return database.MakeError(database.ErrDriverSpecific, str, nil)
//	}
//
