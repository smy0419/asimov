...
func (c *dbCache) updateDB(fn func(ldbTx *leveldb.Transaction) error) error {
	...
	if err != nil {
		return database.ConvertErr("failed to open ldb transaction", err)
	}
	...
	if err := ldbTx.Commit(); err != nil {
		return database.ConvertErr("failed to commit leveldb transaction", err)
	}
	return nil
}
		...
		pendingKeys.ForEach(func(k, v []byte) bool {
			if dbErr := ldbTx.Put(k, v, nil); dbErr != nil {
				str := fmt.Sprintf("failed to put key %q to "+
					"ldb transaction", k)
				innerErr = database.ConvertErr(str, dbErr)
				return false
			}
			return true
		})
