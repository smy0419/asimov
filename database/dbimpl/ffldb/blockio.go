...
func (s *blockStore) openWriteFile(fileNum uint32) (filer, error) {
	...
	if err != nil {
		...
		return nil, database.MakeError(database.ErrDriverSpecific, str, err)
	}
...
func (s *blockStore) openFile(fileNum uint32) (*lockableFile, error) {
	...
	if err != nil {
		return nil, database.MakeError(database.ErrDriverSpecific, err.Error(),
			err)
	}
...
func (s *blockStore) deleteFile(fileNum uint32) error {
	...
	if err := os.Remove(filePath); err != nil {
		return database.MakeError(database.ErrDriverSpecific, err.Error(), err)
	}
...
func (s *blockStore) writeData(data []byte) error {
	wc := s.writeCursor
	n, err := wc.curFile.file.WriteAt(data, int64(wc.curOffset))
	wc.curOffset += uint32(n)
	if err != nil {
		str := fmt.Sprintf("failed to write data to file %d at "+
			"offset %d: %v", wc.curFileNum, wc.curOffset-uint32(n), err)
...
func (s *blockStore) writeBlock(rawBlock []byte) (blockLocation, error) {
	...
	if err := s.writeData(scratch[:]); err != nil {
		return blockLocation{}, err
	}
	...
	// Serialized block.
	if err := s.writeData(rawBlock[:]); err != nil {
		return blockLocation{}, err
	}
	_, _ = hasher.Write(rawBlock)

	// Castagnoli CRC-32 as a checksum of all the previous.
	if err := s.writeData(hasher.Sum(nil)); err != nil {
		return blockLocation{}, err
	}
...
//
// Format: <network><block length><serialized block><checksum>
func (s *blockStore) readBlock(key *database.BlockKey, loc blockLocation) ([]byte, error) {
	...
	blockFile.RUnlock()
	if err != nil {
		str := fmt.Sprintf("failed to read block %v from file %d, "+
			"offset %d: %v", key, loc.blockFileNum, loc.fileOffset,
			err)
		return nil, database.MakeError(database.ErrDriverSpecific, str, err)
	}
	...
	if serializedChecksum != calculatedChecksum {
		str := fmt.Sprintf("block data for block %v checksum "+
			"does not match - got %x, want %x", key,
	...
	if serializedNet != uint32(s.network) {
		str := fmt.Sprintf("block data for block %v is for the "+
			"wrong network - got %d, want %d", key, serializedNet,
			uint32(s.network))
		return nil, database.MakeError(database.ErrDriverSpecific, str, nil)
	}
...
func newBlockStore(basePath string, network common.AsimovNet) *blockStore {
