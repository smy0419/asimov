...
type transaction struct {
	...
	// kept to allow quick lookups of pending data by block key.
	pendingBlocks    map[database.BlockKey]int
...
func (tx *transaction) checkClosed() error {
	...
	if tx.closed {
		return database.MakeError(database.ErrTxClosed, errTxClosedStr, nil)
	}
...
func (tx *transaction) hasBlock(key *database.BlockKey) bool {
	// Return true if the block is pending to be written on commit since
	// it exists from the viewpoint of this transaction.
	if _, exists := tx.pendingBlocks[*key]; exists {
		return true
	}

	return tx.hasKey(bucketizedKey(blockIdxBucketID, key[:]))
}
...
// HasBlock returns whether or not a block with the given key exists in the
// database.
//
// Returns the following errors as required by the interface contract:
//   - ErrTxClosed if the transaction has already been closed
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) HasBlock(key *database.BlockKey) (bool, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return false, err
	}

	return tx.hasBlock(key), nil
}
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) HasBlocks(keys []database.BlockKey) ([]bool, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, err
	}
...
// key.  It will return ErrBlockNotFound if there is no entry.
func (tx *transaction) fetchBlockRow(key *database.BlockKey) ([]byte, error) {
	blockRow := tx.blockIdxBucket.Get(key[:])
	if blockRow == nil {
		str := fmt.Sprintf("block %v does not exist", key)
		return nil, database.MakeError(database.ErrBlockNotFound, str, nil)
	}
...
// identified by the given key.  The raw bytes are in the format returned by
// Serialize on a protos.BlockHeader.
//
// Returns the following errors as required by the interface contract:
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) FetchBlockHeader(key *database.BlockKey) ([]byte, error) {
...
// identified by the given keys.  The raw bytes are in the format returned by
// Serialize on a protos.BlockHeader.
//
// Returns the following errors as required by the interface contract:
//   - ErrBlockNotFound if the any of the requested block keys do not exist
//   - ErrTxClosed if the transaction has already been closed
//   - ErrCorruption if the database has somehow become corrupted
//
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) FetchBlockHeaders(keys []database.BlockKey) ([][]byte, error) {
	regions := make([]database.BlockRegion, len(keys))
	for i := range keys {
		regions[i].Key = &keys[i]
		regions[i].Offset = 0
...
// given key.  The raw bytes are in the format returned by Serialize on a
// protos.MsgBlock.
//
// Returns the following errors as required by the interface contract:
//   - ErrBlockNotFound if the requested block key does not exist
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) FetchBlock(key *database.BlockKey) ([]byte, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, err
	}
	...
	blockBytes, err := tx.db.store.readBlock(key, location)
	if err != nil {
		return nil, err
	}
...
// given keys.  The raw bytes are in the format returned by Serialize on a
// protos.MsgBlock.
//
// Returns the following errors as required by the interface contract:
//   - ErrBlockNotFound if any of the requested block do not exist
//   - ErrTxClosed if the transaction has already been closed
//   - ErrCorruption if the database has somehow become corrupted
//
// In addition, returns ErrDriverSpecific if any failures occur when reading the
...
//
// This function is part of the database.Tx interface implementation.
func (tx *transaction) FetchBlocks(keys []database.BlockKey) ([][]byte, error) {
	// Ensure transaction state is valid.
	if err := tx.checkClosed(); err != nil {
		return nil, err
	}
	...
	for i := range keys {
		var err error
		blocks[i], err = tx.FetchBlock(&keys[i])
		if err != nil {
			return nil, err
		}
	}
...
func (tx *transaction) fetchPendingRegion(region *database.BlockRegion) ([]byte, error) {
	...
	idx, exists := tx.pendingBlocks[*region.Key]
	if !exists {
		return nil, nil
	}
	...
	if endOffset < region.Offset || endOffset > blockLen {
		str := fmt.Sprintf("block %v region offset %d, length %d "+
			"exceeds block length of %d", region.Key,
			region.Offset, region.Len, blockLen)
		return nil, database.MakeError(database.ErrBlockRegionInvalid, str, nil)
	}
...
// load entire blocks.
//
// The raw bytes are in the format returned by Serialize on a protos.MsgBlock and
...
func (tx *transaction) FetchBlockRegion(region *database.BlockRegion) ([]byte, error) {
	...
	// Lookup the location of the block in the files from the block index.
	blockRow, err := tx.fetchBlockRow(region.Key)
	if err != nil {
		return nil, err
	}
	location := deserializeBlockLoc(blockRow)
...
//
// Returns the following errors as required by the interface contract:
//   - ErrBlockNotFound if any of the request block keyes do not exist
...
func (tx *transaction) FetchBlockRegions(regions []database.BlockRegion) ([][]byte, error) {
	...
	// the block location for all of the requested block keys and sort
	...
	// performance increase depending on how spread out the requested keys
	...
	for i := range regions {
		...
		endOffset := region.Offset + region.Len
		if endOffset < region.Offset || endOffset > location.blockLen {
			str := fmt.Sprintf("block %v region offset %d, length "+
				"%d exceeds block length of %d", region.Key,
				region.Offset, region.Len, location.blockLen)
			return nil, database.MakeError(database.ErrBlockRegionInvalid, str, nil)
		}
